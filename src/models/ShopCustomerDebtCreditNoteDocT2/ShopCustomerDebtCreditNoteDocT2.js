/**
 * A function do dynamics table of model ShopCustomerDebtCreditNoteDocT2
 * @param {string} table_name - Dynamics table's name of "dat_${table_name}_customer_debt_cn_doc"
 */
const ShopCustomerDebtCreditNoteDocT2 = (table_name) => {
    ...
    class ShopCustomerDebtCreditNoteDocT2 extends Model {
        static async createShopCustomerDebtCreditNote_Doc(shopId = null, userId = null, shopCustomerDebtCreditNoteDoc = null, options = {}) {
            ...
            const createdDocument = ShopCustomerDebtCreditNoteDocT2.create(
                objToCreate,
                {
                    validate: true,
        ...
        static async updateShopCustomerDebtCreditNote_Doc(shopId = null, userId = null, shopCustomerDebtCreditNoteDoc = null, options = {}) {
            ...
            const findDocument = await ShopCustomerDebtCreditNoteDocT2.findOne({
                where: {
            ...
            if (!findDocument) { throw new Error(`ไม่พบข้อมูลเอกสารใบลดหนี้ของลูกหนี้การค้าจากการแก้ไขเอกสาร`); }
                ...
                if (findDocument.previous('status') === 1 && (shopCustomerDebtCreditNoteDoc?.status === 0 || shopCustomerDebtCreditNoteDoc?.status === 2)) {
                    return await ShopCustomerDebtCreditNoteDocT2.cancelShopCustomerDebtCreditNote_Doc(
                        shopId,
                        userId,
                        findDocument.get('id'),
                        {
        ...
        static async createOrUpdateShopCustomerDebtCreditNote_Doc_Lists(shopId = null, userId = null, shopCustomerDebtCreditNoteDoc = null, shopCustomerDebtCreditNoteLists = null, options = {}) {
             ...
             *  isCreated: boolean;
             *  isUpdated: boolean;
             *  previousData: ShopCustomerDebtCreditNoteDocT2 | null;
             *  currentData: ShopCustomerDebtCreditNoteDocT2;
             * }
             * }
             */
            let shopCustomerDebtCreditNoteDoc__Data = null;
            if (!shopCustomerDebtCreditNoteDoc?.id) {
                const createdDocument = await ShopCustomerDebtCreditNoteDocT2.createShopCustomerDebtCreditNote_Doc(
                    shopId,
                    userId,
                    shopCustomerDebtCreditNoteDoc,
                    {
                ...
                if (!isUUID(shopCustomerDebtCreditNoteDoc.id)) {
                    ...
                    const updatedDocument = await ShopCustomerDebtCreditNoteDocT2.updateShopCustomerDebtCreditNote_Doc(
                        shopId,
                        userId,
                        shopCustomerDebtCreditNoteDoc,
                        {
                    ...
                    if (shopCustomerDebtCreditNoteDoc__Data.isUpdated && shopCustomerDebtCreditNoteDoc__Data.currentData.get('status') !== 1) {
                        return {
                            ShopCustomerDebtCreditNoteDocT2: shopCustomerDebtCreditNoteDoc__Data,
                            ShopCustomerDebtCreditNoteLists: (await ShopCustomerDebtCreditNoteListT2.findAll({
                                where: {
                                    shop_customer_debt_cn_doc_id_t2: shopCustomerDebtCreditNoteDoc__Data.currentData.get('id'),
                                    status: 1
                                },
                                transaction: transaction,
                                ShopModels: ShopModels
            ...
            else { throw new Error(`ไม่สามารถสร้างหรือแก้ไขข้อมูลเอกสารใบลดหนี้ของลูกหนี้การค้าได้เนื่องจากการส่งชุดข้อมูลฟิวส์ไม่ตรงตามเงื่อนไข`); }
            ...
            if (shopCustomerDebtCreditNoteDoc__Data?.isCreated === true && shopCustomerDebtCreditNoteDoc__Data?.isUpdated === false) {
                shopCustomerDebtCreditNoteLists__Data = await ShopCustomerDebtCreditNoteListT2.createOrUpdateShopCustomerDebtCreditNote_Lists(
                    shopId,
                    userId,
    ...
    ShopCustomerDebtCreditNoteDocT2.init(
        {
            id: {
        ...
        {
            sequelize: db,
            modelName: 'ShopCustomerDebtCreditNoteDocT2',
            schema: 'app_shops_datas',
            tableName: `dat_${table_name}_customer_debt_cn_doc_t2`,
            comment: 'ตารางข้อมูลเอกสารใบลดหนี้ของลูกหนี้การค้า',
            timestamps: false,
            indexes: [
                {
                    name: `idx_${table_name}_ccn_doc_code_id_t2`,
                    fields: ['code_id']
                },
                {
                    name: `idx_${table_name}_ccn_doc_type_id_t2`,
                    fields: ['doc_type_id']
                },
                {
                    name: `idx_${table_name}_ccn_bus_customer_id_t2`,
                    fields: ['bus_customer_id']
                },
                {
                    name: `idx_${table_name}_ccn_per_customer_id_t2`,
                    fields: ['per_customer_id']
                },
                {
                    name: `idx_${table_name}_ccn_tmp_doc_id_t2`,
                    fields: ['shop_temporary_delivery_order_doc_id']
                },
                {
                    name: `idx_${table_name}_ccn_tax_type_id_t2`,
                    fields: ['tax_type_id']
                }
            ]
        }
    ...
    ShopCustomerDebtCreditNoteDocT2.belongsTo(ShopProfile, { foreignKey: 'shop_id', as: 'ShopProfile' });
    ShopCustomerDebtCreditNoteDocT2.belongsTo(DocumentType, { foreignKey: 'doc_type_id', as: 'DocumentType' });
    ShopCustomerDebtCreditNoteDocT2.belongsTo(ShopBusinessCustomer, { foreignKey: 'bus_customer_id', as: 'ShopBusinessCustomer' });
    ShopCustomerDebtCreditNoteDocT2.belongsTo(ShopPersonalCustomer, { foreignKey: 'per_customer_id', as: 'ShopPersonalCustomer' });
    ShopCustomerDebtCreditNoteDocT2.belongsTo(ShopTemporaryDeliveryOrderDoc, { foreignKey: 'shop_temporary_delivery_order_doc_id', as: 'ShopTemporaryDeliveryOrderDoc' });
    ShopCustomerDebtCreditNoteDocT2.belongsTo(TaxType, { foreignKey: 'tax_type_id', as: 'TaxType' });
    ShopCustomerDebtCreditNoteDocT2.belongsTo(User, { foreignKey: 'created_by', as: 'CreatedBy' });
    ShopCustomerDebtCreditNoteDocT2.belongsTo(User, { foreignKey: 'updated_by', as: 'UpdatedBy' });
    ...
    const hookFunctions = (options = {}) => {
        ...
        /**
         * @param {ShopCustomerDebtCreditNoteDocT2} instance
         ...
         * @param {ShopCustomerDebtCreditNoteDocT2} instance
         * @param {import("sequelize/types/model").UpdateOptions<ShopCustomerDebtCreditNoteDocT2> | import("sequelize/types/model").SaveOptions<ShopCustomerDebtCreditNoteDocT2>} options
         */
         ...
         * @param {ShopCustomerDebtCreditNoteDocT2} instance
         * @param {(import("sequelize/types/model").UpdateOptions<ShopCustomerDebtCreditNoteDocT2> | import("sequelize/types/model").SaveOptions<ShopCustomerDebtCreditNoteDocT2>) & { isCancelStatus_Doc?: boolean }} options
         */
        const hookBeforeSave_checkUsingThisDocumentIfThisDocumentSetToChangeOrCancel = async (instance, options) => {
            if ((!instance.isNewRecord && instance.changed()) || options?.isCancelStatus_Doc === true) {
                ...
                const findShopCustomerDebtList = await ShopCustomerDebtList.findOne({
                    ...
                    where: {
                        shop_customer_debt_cn_doc_id_t2: instance.get('id'),
                        status: 1
                    },
                    transaction: transaction
                });
    ...
    ShopCustomerDebtCreditNoteDocT2.beforeValidate(async (instance, options) => {
    ...
    ShopCustomerDebtCreditNoteDocT2.beforeSave(async (instance, options) => {
    ...
    return ShopCustomerDebtCreditNoteDocT2;
};


module.exports = ShopCustomerDebtCreditNoteDocT2;