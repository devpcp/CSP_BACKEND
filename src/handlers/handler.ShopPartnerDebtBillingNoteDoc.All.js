 ...
 */
const handlerShopPartnerDebtBillingNoteDocAll = async (request = {}, reply = {}, options = {}) => {
    const action = 'GET ShopPartnerDebtBillingNoteDoc.All';
    ...
    try {
        ...
        const whereQuery = !search
            ...
            : {
                ...
                [Op.or]: [
                    ...
                    literal(`
                        ((
                            SELECT DISTINCT X.shop_partner_debt_bn_doc_id
                            FROM ${ShopPartnerDebtBillingNoteList.options.schema}.${ShopPartnerDebtBillingNoteList.options.tableName} AS X
                            WHERE (
                                (
                                    SELECT id
                                    FROM ${ShopPartnerDebtBillingNoteDoc.options.schema}.${ShopPartnerDebtBillingNoteDoc.options.tableName} AS Y
                                    WHERE Y.id = X.shop_service_order_doc_id
                                        AND Y.code_id iLIKE '%${search}%'
                                ) = X.shop_service_order_doc_id
                            )
                            AND X.status = 1
                            AND X.shop_partner_debt_bn_doc_id = "ShopPartnerDebtBillingNoteDoc".id
                        ) = "ShopPartnerDebtBillingNoteDoc".id)
                    `.replace(/(\s)+/g, ' ')),
                    {
        ...
        const fnFindDocuments = async () => await ShopPartnerDebtBillingNoteDoc.findAll({
            attributes: {
                include: [
                    [literal(`(SELECT user_name FROM systems.sysm_users WHERE id = "ShopPartnerDebtBillingNoteDoc"."created_by" )`), 'created_by'],
                    [literal(`(SELECT user_name FROM systems.sysm_users WHERE id = "ShopPartnerDebtBillingNoteDoc"."updated_by" )`), 'updated_by'],
                ]
            },
        ...
        const fnFindDocuments_Count = async () => await ShopPartnerDebtBillingNoteDoc.count({
            include: includeQuery,
            where: whereQuery,
            transaction: request?.transaction || options?.transaction || null,
            ShopModels: ShopModels
...
module.exports = handlerShopPartnerDebtBillingNoteDocAll;