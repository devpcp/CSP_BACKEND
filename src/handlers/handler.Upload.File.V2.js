        ...
        if (!_.isPlainObject(fileUploadParser)) {
            ...
            if (!existsSync(directoryRequestPath)) {
                ...
                const saveImage = await utilSetImageFileConvert(await fileUploadParser.toBuffer()).then(
                    async (jimpResult) => {
                        if (jimpResult) {
                            return await utilSetImageFileSave(jimpResult, directoryIdRequestPath, `${fileName}.png`)
                        }
                    }
                ).catch(
                    async (e) => {
                ...
                if (!saveImage) {
                    ...
                    await handleSaveLog(request, [["post upload", '', `/assets/${fileDirectory}/${fileDirectoryId}/${fileName}.png`], ""]);
                    return { status: 'success', data: { path: `/assets/${fileDirectory}/${fileDirectoryId}/${fileName}.png` } };
                }
            }
        }
    } catch (error) {
