...
const printOutPdf = async (request, reply, app) => {
    ...
    try {
        ...
        */
        ...
        tran_doc.shop_profile = shop_profile
        tran_doc.vehicle_data_show = request.query.vehicle_data_show
        tran_doc.doc_date_show = request.query.doc_date_show
        ...
        //because doc_type_id always null, i don't know why.
        if (tran_doc.doc_type_code_id == 'TRN' && config_pdf.delivery_doc?.logo?.isuse == false) {
            logo = null

        }

        if (tran_doc.doc_type_code_id == 'TRN') {
            if (config_pdf.delivery_doc?.font_primary_color) {
                config_pdf.font_primary_color = config_pdf.delivery_doc?.font_primary_color
            }
            if (config_pdf.delivery_doc?.header_font_color) {
                config_pdf.header_font_color = config_pdf.delivery_doc?.header_font_color
            }
            if (config_pdf.delivery_doc?.header_table_color) {
                config_pdf.header_table_color = config_pdf.delivery_doc?.header_table_color
                header_table_color = config_pdf.header_table_color
            }
        }
        ...
        let province = (addr_replare[0] === ' - ') ? `${addr_replare[3][2]} ${addr_replare[0]}` : `${addr_replare[3][2]}${addr_replare[0]}`
        ...
        // copy front 
        //ไม่รวม vat// รวม vat
        if (['fafa3667-55d8-49d1-b06c-759c6e9ab064', '8c73e506-31b5-44c7-a21b-3819bb712321'].includes(tran_doc.tax_type_id) || !tran_doc.tax_type_id) {
            addr = addr + `\nเลขประจำตัวผู้เสียภาษีอากร: ${(shop_profile.tax_code_id) ? shop_profile.tax_code_id.split('(')[0] : ' - '} `
        } else {
        ...
        if (tran_doc.which == 1) {
            ...
            for (let index = 0; index < product_all.length; index++) {
                ...
                data.push({
                    ...
                    ... (tran_doc.price_use == 'true') ? (config_pdf.all?.discount == 'percent') ? { 'ส่วนลด': (parseFloat(element.price_discount_percent || 0)).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") } : { 'ส่วนลด': (parseFloat(discount || 0) * parseInt(element.amount || 0)).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") } : {},
            ...
            if (tran_doc.price_use == 'true') {
                ...
                if (tran_doc.which == 1 && tran_doc.ShopVehicleCustomer != null && tran_doc.vehicle_data_show == true) {
            ...
            var header_left = [
                ...
                { 'วันที่เอกสาร': (tran_doc.doc_date_show == true) ? moment(new Date(tran_doc.doc_date)).format("DD/MM/YYYY") : '' },
            ...
            if (config_pdf.seller_show?.isuse != false) {
                if (config_pdf.all?.sales_man == true) {
                    let sales_man = null
                    if (tran_doc.details?.sales_man) {
                        sales_man = await UsersProfiles.findAll({ where: { user_id: tran_doc.details?.sales_man } })
                    }
            ...
            if (config_pdf.repair_man_show?.isuse != false) {
                ...
                if (config_pdf.template?.value == 2 && tran_doc.doc_sales_type == 2) {
                    ...
                    if (config_pdf.all?.repair_man == false) {
            ...
            if (config_pdf.all?.show_time == true) {

                header_left.push({ 'เวลา': moment(new Date()).format("DD/MM/YYYY HH::mm") })
            ...
            if (config_pdf.repair_doc?.payment_show?.value == true && tran_doc.which_step == 1) {
                ...
                if (config_pdf.repair_doc?.created_by?.value == false) {
                    ...
                    let do_doc = null
                    if (tran_doc.details?.user_id) {
                        do_doc = await UsersProfiles.findOne({ where: { user_id: tran_doc.details?.user_id } })
                    }
            ...
            // ถ้าเป็นใบรับคืนสินค้า
            if (tran_doc.doc_type_id == 'dfa5a7ed-4f01-41bd-894f-cd28d1068ad0') {

                if (tran_doc.details?.customer_type == 'personal') {
                    partner = await ShopPersonalCustomers(table_name).findOne({
                        attributes: {
                            include: [[sequelize.literal(`json_build_object('th',concat(customer_name->'first_name'->>'th',' ',customer_name->'last_name'->>'th'))`), 'partner_name'], ['id_card_number', 'tax_id']]
                        },
                        where: { id: tran_doc.details.customer_id }
                    })

                } else {
                    partner = await ShopBusinessCustomers(table_name).findOne({
                        attributes: {
                            include: [['customer_name', 'partner_name']]
                        },
                        where: { id: tran_doc.details.customer_id }
                    })
                ...
                tran_doc.ShopBusinessPartners = partner
                tran_doc.seller_title = 'ข้อมูลลูกค้า'
            ...
            var header_left = [
                ...
                // { 'วันครบกำหนดชำระ': '' },
                { 'ผู้สั่งซื้อ': (config_pdf.all?.order_name == "use_branch_name") ? tran_doc.shop_profile.shop_name.shop_local_name : user_profile.fname.th + ' ' + user_profile.lname.th }
            ]

            // if(tran_doc.doc_type_id == '53e7cbcc-3443-40e5-962f-d9512aba2b5a' ||tran_doc.doc_type_id == '4979e859-92d1-4485-9243-45cdd505adb8' ){
            ...
            if (tran_doc.doc_type_id == '53e7cbcc-3443-40e5-962f-d9512aba2b5a') {
                in_name = ['ในนาม ' + shop_profile.shop_name.th, 'ในนาม ' + name2]
            }
            ...
            if (tran_doc.price_use == 'true') {
                ...
                let branch_ = customer.other_details || {}
                if (config_pdf.quotation_doc?.show_branch) {
                    if (customer.other_details.branch == 'office') {
                        addr1 = addr1 + ` (สำนักงานใหญ่)\n`
                ...
                if (config_pdf.quotation_doc?.show_tax == true) {
                    addr1 = addr1 + `\nเลขประจำตัวผู้เสียภาษี ${(customer.tax_id) ? customer.tax_id : ' - '} `
                }
            ...
            let branch_ = customer.other_details || {}
            if (config_pdf.customer_debt_doc?.show_branch) {
                if (customer.other_details.branch == 'office') {
                    addr1 = addr1 + ` (สำนักงานใหญ่)\n`
            ...
            if (config_pdf.customer_debt_doc?.show_tax == true) {
                addr1 = addr1 + `\nเลขประจำตัวผู้เสียภาษี ${(customer.tax_id) ? customer.tax_id : ' - '} `
            }
            seller = { title: 'ข้อมูลลูกค้า', data: addr1 }
            ...
            let branch_ = customer.other_details || {}
            if (config_pdf.customer_debt_billingnote_doc?.show_branch) {
                if (customer.other_details.branch == 'office') {
                    addr1 = addr1 + ` (สำนักงานใหญ่)\n`
            ...
            if (config_pdf.customer_debt_billingnote_doc?.show_tax == true) {
                addr1 = addr1 + `\nเลขประจำตัวผู้เสียภาษี ${(customer.tax_id) ? customer.tax_id : ' - '} `
            }
            seller = { title: 'ข้อมูลลูกค้า', data: addr1 }
...
const pdfGen = async (logo, header_right, header_left, seller, send, ws, price, note, in_name, name, sign_date, bank, use_sign_date, tran_doc, shop_table, config_pdf) => {
    ...
    if (tran_doc.doc_type_code_id == 'TRN' && config_pdf.delivery_doc?.show_bank == true) {
        header_left.push({ 'โอนเข้า บ/ช': 'กสิรไทย บ.ศุภชัยยางยนต์ 0138-11-6069' })
    }
...
const template_default = async (logo, header_right, header_left, seller, send, ws, price, note, in_name, name, sign_date, bank, use_sign_date, tran_doc, shop_table, config_pdf) => {
    ...
    for (let i = 0; i < pages.count; i++) {
        ...
        if (Object.keys(seller).length > 0) {
            ...
            doc.font(Bold).fontSize(font_customer_size).fillColor('black').text(seller.data, { width: 280 })
        }
...
const template_1 = async (logo, header_right, header_left, seller, send, ws, price, note, in_name, name, sign_date, bank, use_sign_date, tran_doc, shop_table, config_pdf) => {
    ...
    if (tran_doc.price_use == 'true') {
        ...
        doc.font(Bold).fontSize(font_customer_size).text(pric_all?.toString().replace(' บาท', ''), 0, 23.65 * px, { align: 'right', width: 19.2 * px });

        doc.font(Bold).fontSize(font_customer_size).text(ArabicNumberToText(pric_all), 3 * px, 23.65 * px);
    ...
    for (let i = 0; i < pages.count; i++) {
        ...
        let doc_date = (tran_doc.doc_date_show == true) ? moment(new Date(tran_doc.doc_date)).format("DD/MM/YYYY") : ''
