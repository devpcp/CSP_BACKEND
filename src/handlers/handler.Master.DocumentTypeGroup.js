...
const handleAllRaw = async (request) => {
    const handlerName = 'get master document type group all raw';
    ...
    try {
        ...
        var attr = {
            include: [
                [literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"DocumentTypeGroups\".\"created_by\" )"), 'created_by'],
                [literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"DocumentTypeGroups\".\"updated_by\" )"), 'updated_by'],
            ]
        }
        ...
        var data = await DocumentTypeGroups.findAll({
            attributes: attr,
            where: where_q,
            order: [[sort, order]]
        })
...
const handleAll = async (request) => {
    const handlerName = 'get master document type group all';
    ...
    try {
        ...
        var length_data = await DocumentTypeGroups.count({
            attributes: attr,
            where: where_q
        })
...
const handleById = async (request) => {
    const handlerName = 'get master document type group byid';

    var id = request.params.id
    var data = await DocumentTypeGroups.findOne({
        attributes: {
            include: [
                [literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"DocumentTypeGroups\".\"created_by\" )"), 'created_by'],
                [literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"DocumentTypeGroups\".\"updated_by\" )"), 'updated_by'],
    ...
    if (data) {
        ...
        await handleSaveLog(request, [[handlerName], 'document type group not found'])
        return utilSetFastifyResponseJson("failed", "document type group not found");
    }
}

const handleAdd = async (request) => {

    const handlerName = 'add master document type group';

    try {

        var createRunNumber = await utilGetRunNumberFromModel(DocumentTypeGroups, 'run_no', { prefix_config: 'GDOC-' })

        var create = await DocumentTypeGroups.create({
            ...request.body,
...
const handlePut = async (request) => {
    ...
    const handlerName = 'put master document type group';
    ...
    try {
        ...
        const find = await DocumentTypeGroups.findAll({ where: { id: id } });
        if (!find[0]) {
        ...
        var before_update = await DocumentTypeGroups.findOne({
            where: {
                id: id
            }
        });

        await DocumentTypeGroups.update(data, {
            where: {
                id: id
            }
        });

        const put = await DocumentTypeGroups.findOne(
            {
                where: {
                    id: id
                }
