...
const {
    ...
    config_run_number_master_business_type_prefix
} = require('../config');
 ...
 */
const handlerShopBusinessCustomersAddByFile = async (request) => {

    var action = "add shopBusinessCustomers by file"
    try {
        ...
        let customer_all = await instanceModelShopBusinessCustomers.findAll({})
        ...
        const findLatestRowNumber = await instanceModelShopBusinessCustomers.max(
            'run_no',
            {
                where: {
                    ['created_date']: {
        ...
        for (let index = 0; index < data_json.length; index++) {
            ...
            let check_existing = customer_all.filter(el => {
                if (element["รหัสลูกค้าจากระบบเดิม"]) {
                    return el.other_details.code_from_old_system === element["รหัสลูกค้าจากระบบเดิม"] &&
                        el.customer_name.th === element.ชื่อธุรกิจ
                } else {
                    return el.customer_name.th === element.ชื่อธุรกิจ && el.tax_id === element["หมายเลขประจำตัวผู้เสียภาษี"]
                }
            ...
            if (check_existing.length > 0) {
                ...
                await instanceModelShopBusinessCustomers.update({
                    ...
                    customer_name: { "th": element.ชื่อธุรกิจ, "en": '' },
                    tel_no: tel_no,
                    mobile_no: mobile_no,
        ...
        let data_create_id_arr_ = await instanceModelShopBusinessCustomers.bulkCreate(data_create);
        ...
        let customer_from_hq = await instanceModelShopBusinessCustomers.findAll({
            where: {
        ...
        for (let index = 0; index < findShopsProfileArray.length; index++) {
            ...
            if (element.shop_code_id !== table_name) {
                ...
                let customer_all = await modelShopBusinessCustomers(element.shop_code_id).findAll()
                let data_create1 = []
                for (let index1 = 0; index1 < customer_from_hq.length; index1++) {
                    ...
                    let check_existing = customer_all.filter(el => {
                        return el.other_details.code_from_old_system === element1.other_details.code_from_old_system &&
                            ...
                            el.customer_name.th === element1.customer_name.th
                    })
                ...
                await modelShopBusinessCustomers(element.shop_code_id).bulkCreate(data_create1).then().catch(error => { console.log(error) })
...
module.exports = handlerShopBusinessCustomersAddByFile;