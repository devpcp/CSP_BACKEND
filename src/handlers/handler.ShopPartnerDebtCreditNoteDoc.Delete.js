...
/**
 * - Route [DELETE] => /api/shopPartnerDebtCreditNoteDoc/delete
 * @template T
 ...
 */
const handlerShopPartnerDebtCreditNoteDocDelete = async (request = {}, reply = {}, options = {}) => {
    const action = 'DELETE ShopPartnerDebtCreditNoteDoc.Delete';
        ...
        const transactionResult = await db.transaction(
            ...
            async (transaction) => {
                ...
                if (!findShopPartnerDebtCreditNoteDoc) {
                    throw new Error(`ไม่พบข้อมูลเอกสารใบลดหนี้ของเจ้าหนี้การค้า`);
                }
                else if (findShopPartnerDebtCreditNoteDoc.get('status') === 0) {
                    throw new Error(`ไม่อนุญาตให้ยกเลิกเอกสารใบลดหนี้ของเจ้าหนี้การค้า เนื่องจากถูกยกเลิกไปก่อนแล้ว`);
                }
                else {
                    const canceledDocument = await ShopPartnerDebtCreditNoteDoc.cancelShopPartnerDebtCreditNote_Doc(
                        findShopPartnerDebtCreditNoteDoc.get('shop_id'),
                        request.id,
                        findShopPartnerDebtCreditNoteDoc.get('id'),
                        {
                            ...options,
...
module.exports = handlerShopPartnerDebtCreditNoteDocDelete;