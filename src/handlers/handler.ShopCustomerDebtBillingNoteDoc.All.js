 ...
 */
const handlerShopCustomerDebtBillingNoteDocAll = async (request = {}, reply = {}, options = {}) => {
    const action = 'GET ShopCustomerDebtBillingNoteDoc.All';
    ...
    try {
        ...
        const whereQuery = !search
            ...
            : {
                ...
                [Op.or]: [
                    ...
                    literal(`
                        ((
                            SELECT DISTINCT X.shop_customer_debt_bn_doc_id
                            FROM ${ShopCustomerDebtBillingNoteList.options.schema}.${ShopCustomerDebtBillingNoteList.options.tableName} AS X
                            WHERE (
                                (
                                    SELECT id
                                    FROM ${ShopCustomerDebtBillingNoteDoc.options.schema}.${ShopCustomerDebtBillingNoteDoc.options.tableName} AS Y
                                    WHERE Y.id = X.shop_service_order_doc_id
                                        AND Y.code_id iLIKE '%${search}%'
                                ) = X.shop_service_order_doc_id
                            )
                            AND X.status = 1
                            AND X.shop_customer_debt_bn_doc_id = "ShopCustomerDebtBillingNoteDoc".id
                        ) = "ShopCustomerDebtBillingNoteDoc".id)
                    `.replace(/(\s)+/g, ' ')),
                    {
        ...
        const fnFindDocuments = async () => await ShopCustomerDebtBillingNoteDoc.findAll({
            attributes: {
                include: [
                    [literal(`(SELECT user_name FROM systems.sysm_users WHERE id = "ShopCustomerDebtBillingNoteDoc"."created_by" )`), 'created_by'],
                    [literal(`(SELECT user_name FROM systems.sysm_users WHERE id = "ShopCustomerDebtBillingNoteDoc"."updated_by" )`), 'updated_by'],
                ]
            },
        ...
        const fnFindDocuments_Count = async () => await ShopCustomerDebtBillingNoteDoc.count({
            include: includeQuery,
            where: whereQuery,
            transaction: request?.transaction || options?.transaction || null,
            ShopModels: ShopModels
...
module.exports = handlerShopCustomerDebtBillingNoteDocAll;