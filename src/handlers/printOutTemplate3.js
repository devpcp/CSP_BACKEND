...
const printOutTemplate3 = async (request, tran_doc) => {
...
const pdfGen = async (doc_type_name, customer, vehicle, ws, price_use, price, note, tran_doc, config_pdf) => {
    ...
    if (tran_doc.which_step == 2) {
        doc = await template_2(doc_type_name, customer, vehicle, ws, price_use, price, note, tran_doc, config_pdf)
...
const template_1 = async (doc_type_name, customer, vehicle, ws, price_use, price, note, tran_doc, config_pdf) => {
    ...
    let doc = new PDFDocument({
        margins: { top: 8 * px, left: 10, right: 7, bottom: 2 * px },
    ...
    ws.headers = [
        ...
        { "label": 'ชื่อสินค้า', "property": 'ชื่อสินค้า', "width": 7.75 * px, },
    ...
    if (price_use == 'true') {
        ...
        doc.font(Regular).fontSize(10).text(note, 3.8 * px, 20.3 * px);

        doc.font(Bold).fontSize(11).text('ขอบพระคุณอย่างสูงที่มาใช้บริการ'.toString(), 4.5 * px, 21.8 * px);

        doc.font(Bold).fontSize(11).text('โอกาสหน้าขอเชิญมาใช้บริการใหม่ค่ะ'.toString(), 4.5 * px, 22.75 * px);


        doc.font(Bold).fontSize(11).text((price['รวมเป็นเงิน'])?.toString(), 0, 21.8 * px, { align: 'right', width: 15 * px });

        doc.font(Bold).fontSize(11).text((price['ส่วนลดรวม'])?.toString(), 0, 22.75 * px, { align: 'right', width: 15 * px });


        doc.font(Bold).fontSize(11).text((price['ราคาก่อนรวมภาษี'] || price['รวมเป็นเงิน'])?.toString(), 0, 21.8 * px, { align: 'right', width: 20.5 * px });

        doc.font(Bold).fontSize(11).text((price['ภาษีมูลค่าเพิ่ม'])?.toString(), 0, 22.75 * px, { align: 'right', width: 20.5 * px });

        doc.font(Bold).fontSize(13).text(pric_all?.toString(), 0, 23.75 * px, { align: 'right', width: 20.5 * px });

        doc.font(Bold).fontSize(13).text(ArabicNumberToText(pric_all), 4 * px, 23.75 * px);
    ...
    for (let i = 0; i < pages.count; i++) {
        ...
        doc.font(Bold).fontSize(10).fillColor('black').text(doc_no, 15.45 * px, 1.85 * px)
        doc.font(Bold).fontSize(10).fillColor('black').text(doc_date, 15.45 * px, 2.65 * px)
        ...
        doc.font(Bold).fontSize(10).fillColor('black').text(doc_date_credit_term, 18 * px, 4.25 * px)
        ...
        if (vehicle?.registration?.includes('\n')) {
            ...
            doc.font(Regular).fontSize(11).fillColor('black').text(vehicle.registration, 2.7 * px, 5.9 * px)

        }

        doc.font(Regular).fontSize(11).fillColor('black').text(vehicle.mileage, 6.3 * px, 5.9 * px)
        doc.font(Regular).fontSize(11).fillColor('black').text(vehicle.brand, 9.5 * px, 5.9 * px)
        doc.font(Regular).fontSize(11).fillColor('black').text(vehicle.model, 12.5 * px, 5.9 * px)
        ...
        if (Object.keys(customer).length > 0) {
            doc.font(Regular).fontSize(12).fillColor('black').text(customer.line1, 2 * px, 2.7 * px)
            doc.font(Regular).fontSize(11).fillColor('black').lineGap(-2).text(customer.line2, 2 * px, 3.2 * px, {
            })
...
const template_2 = async (doc_type_name, customer, vehicle, ws, price_use, price, note, tran_doc, config_pdf) => {
