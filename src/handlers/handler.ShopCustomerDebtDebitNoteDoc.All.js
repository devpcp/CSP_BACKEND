...
/**
 * - Route [GET] => /api/shopCustomerDebtDebitNoteDoc/all
 * @template T
 ...
 */
const handlerShopCustomerDebtDebitNoteDocAll = async (request = {}, reply = {}, options = {}) => {
    const action = 'GET ShopCustomerDebtDebitNoteDoc.All';
    ...
    try {
        ...
        const filter__unUsed__shop_customer_debt_dn_doc_id = request.query?.filter__unUsed__shop_customer_debt_dn_doc_id || false;
        ...
        if (filter__unUsed__shop_customer_debt_dn_doc_id === true) {
            const queryData = literal(
                `
                    ("ShopCustomerDebtDebitNoteDoc".id NOT IN 
                        (
                            SELECT "CDN_Doc".id
                            FROM app_shops_datas.dat_01hq0004_customer_debt_dn_doc AS "CDN_Doc"
                            WHERE "CDN_Doc".status = 1
                            AND ("CDN_Doc".id IN (
                                SELECT "CDN_List".shop_customer_debt_dn_doc_id
                                FROM app_shops_datas.dat_01hq0004_customer_debt_dn_list AS "CDN_List"
                                WHERE "CDN_List".shop_customer_debt_dn_doc_id = "CDN_Doc".id
                                    AND "CDN_List".status = 1
                                    AND ("CDN_List".shop_customer_debt_dn_doc_id IN (
                                        SELECT "CDX_List".shop_customer_debt_dn_doc_id
                                        FROM app_shops_datas.dat_01hq0004_customer_debt_list AS "CDX_List"
                                        WHERE "CDX_List".shop_customer_debt_dn_doc_id = "CDN_List".shop_customer_debt_dn_doc_id
        ...
        const fnFindDocuments = async () => await ShopCustomerDebtDebitNoteDoc.findAll({
            attributes: {
                include: [
                    [literal(`(SELECT user_name FROM systems.sysm_users WHERE id = "ShopCustomerDebtDebitNoteDoc"."created_by" )`), 'created_by'],
                    [literal(`(SELECT user_name FROM systems.sysm_users WHERE id = "ShopCustomerDebtDebitNoteDoc"."updated_by" )`), 'updated_by'],
                ]
            },
        ...
        const fnFindDocuments_Count = async () => await ShopCustomerDebtDebitNoteDoc.count({
            include: includeQuery,
            where: whereQuery,
            transaction: request?.transaction || options?.transaction || null,
            ShopModels: ShopModels
...
module.exports = handlerShopCustomerDebtDebitNoteDocAll;