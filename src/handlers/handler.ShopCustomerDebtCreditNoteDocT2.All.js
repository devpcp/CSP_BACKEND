...
const handlerShopCustomerDebtCreditNoteDocAll = async (request = {}, reply = {}, options = {}) => {
    const action = 'GET ShopCustomerDebtCreditNoteDocT2.All';
    ...
    try {
        ...
        const includeQuery = [
            {
                model: ShopCustomerDebtCreditNoteListT2,
                required: true,
                separate: true,
                attributes: {
                    exclude: ['details']
        ...
        if (filter__unUsed__shop_customer_debt_cn_doc_id === true) {
            const queryData = literal(
                `
                    ("ShopCustomerDebtCreditNoteDocT2".id NOT IN 
                        (
                            SELECT "CCN_Doc".id
        ...
        const fnFindDocuments = async () => await ShopCustomerDebtCreditNoteDocT2.findAll({
            attributes: {
                include: [
                    [literal(`(SELECT user_name FROM systems.sysm_users WHERE id = "ShopCustomerDebtCreditNoteDocT2"."created_by" )`), 'created_by'],
                    [literal(`(SELECT user_name FROM systems.sysm_users WHERE id = "ShopCustomerDebtCreditNoteDocT2"."updated_by" )`), 'updated_by'],
                ]
            },
        ...
        const fnFindDocuments_Count = async () => await ShopCustomerDebtCreditNoteDocT2.count({
            include: includeQuery,
            where: whereQuery,
            transaction: request?.transaction || options?.transaction || null,
            ShopModels: ShopModels
