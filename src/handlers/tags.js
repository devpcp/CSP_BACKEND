...
const handleAll = async (request, res) => {
    ...
    var producr = await Tags.findAll({
        order: [[sort, order]],
        attributes: {
            include: [
                [sequelize.literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"Tags\".\"created_by\" )"), 'created_by'],
                [sequelize.literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"Tags\".\"updated_by\" )"), 'updated_by']
            ]
        },
        required: false,
        where: where_q,
    ...
    var length_data = await Tags.count({
        where: where_q
    })
    ...
    await handleSaveLog(request, [['get tags all'], ''])
...
const handleAdd = async (request, res) => {
    ...
    try {
        ...
        let check_run = await Tags.findOne({
            order: [['run_no', 'desc']]
        })
...
const handleById = async (request, res) => {
    try {
        ...
        var find_product = await Tags.findOne({
            attributes: {
                include: [
                    [sequelize.literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"Tags\".\"created_by\" )"), 'created_by'],
                    [sequelize.literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"Tags\".\"updated_by\" )"), 'updated_by']
        ...
        await handleSaveLog(request, [['get tags byid'], ''])
        return ({ status: "success", data: find_product })


    } catch (error) {
        error = error
        await handleSaveLog(request, [['get tags byid'], 'error : ' + error])
        return ({ status: "failed", data: error })
    }
}
...
const handlePut = async (request, res) => {
    ...
    try {
        ...
        const before_update = await Tags.findOne({ where: { id: id } });
        if (!before_update) {
