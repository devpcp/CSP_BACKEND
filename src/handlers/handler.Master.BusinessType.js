...
const { config_run_number_master_business_type_prefix } = require('../config');
const db = require('../db');
const BusinessType = require('../models/model').BusinessType;

const handleAllRaw = async (request) => {
    const handlerName = 'get master business type all raw';
    ...
    try {
        ...
        var attr = {
            include: [
                [literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"BusinessType\".\"created_by\" )"), 'created_by'],
                [literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"BusinessType\".\"updated_by\" )"), 'updated_by'],
            ]
        }
        ...
        var data = await BusinessType.findAll({
            attributes: attr,
            where: where_q,
            order: [[sort, order]]
        })
...
const handleAll = async (request) => {
    const handlerName = 'get master business type all';
    ...
    try {
        ...
        var length_data = await BusinessType.count({
            attributes: attr,
            where: where_q
        })
...
const handleById = async (request) => {
    const handlerName = 'get master business type byid';

    var id = request.params.id
    var data = await BusinessType.findOne({
        attributes: {
            include: [
                [literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"BusinessType\".\"created_by\" )"), 'created_by'],
                [literal("(SELECT user_name FROM systems.sysm_users WHERE id = \"BusinessType\".\"updated_by\" )"), 'updated_by'],
    ...
    if (data) {
        ...
        await handleSaveLog(request, [[handlerName], 'business type not found'])
        return utilSetFastifyResponseJson("failed", "business type not found");
    }
}

const handleAdd = async (request) => {
    const handlerName = 'add master business type';
    ...
    try {
        const transactionResult = await db.transaction(
            ...
            async (transaction) => {
                ...
                var { internal_code_id, business_type_name } = request.body
                var check_order = await BusinessType.max('order_by') || 0

                if (internal_code_id) {
                    var check = await BusinessType.findOne({
                    ...
                    where: {
                        business_type_name: {
                            [Op.or]: [
                                ...
                                (business_type_name.en) ? { en: business_type_name.en } : {}
                            ]
                        }
                    }
                })
                ...
                const createRunNumber = await utilGetRunNumberFromModel(
                    ...
                    'run_no',
                    {
                        prefix_config: await utilGetDocumentTypePrefix(
                            ...
                            {
                                defaultPrefix: config_run_number_master_business_type_prefix
                            }
                        ).then(r => r.prefix),
                        transaction: transaction
                    }
                );
                const createdDocument = await BusinessType.create(
                    {
                        ...request.body,
...
const handlePut = async (request) => {
    const handlerName = 'put master business type';
    ...
    try {
        const transactionResult = await db.transaction(
            ...
            async (transaction) => {
                ...
                const beforeUpdateDocument = await BusinessType.findOne({
                    where: {
                ...
                const updatedDocument = await BusinessType.update(
                    {
                        ...request.body,
                ...
                const afterUpdateDocument = await BusinessType.findOne({
                    where: {
                        id: request.params.id
                    },
                    transaction: transaction
