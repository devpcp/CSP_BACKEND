 ...
 */
const handlerShopPartnerDebtDocPut = async (request = {}, reply = {}, options = {}) => {
    const action = 'PUT ShopPartnerDebtDoc.Put';
            ...
            const transactionResult = await db.transaction(
                ...
                async (transaction) => {
                    ...
                    if (request.body?.status === 2) {
                        const canceledDocument = await ShopPartnerDebtDoc.cancelShopPartnerDebt_Doc(
                            request.id,
                            findShopPartnerDebtDoc.get('id'),
                            {
                    ...
                    const created_and_updated_doc_lists = await ShopPartnerDebtDoc.editShopPartnerDebt_Doc_Lists(
                        findShopPartnerDebtDoc.get('shop_id'),
                        request.id,
                        findShopPartnerDebtDoc.get('id'),
                        request.body,
                        request.body?.shopPartnerDebtLists || [],
                        {
                            currentDateTime: currentDateTime,
                            transaction: transaction,
                            ShopModels: ShopModels
...
module.exports = handlerShopPartnerDebtDocPut;