    ...
    const transactionResults = await db.transaction(
        ...
        async (transaction) => {
            for (let indexShop = 0; indexShop < shop_code_ids.length; indexShop++) {
                ...
                await db.query(
                    `
                                ALTER TABLE app_shops_datas.dat_01hq0004_products ADD COLUMN IF NOT EXISTS price_dot_arr json[] NULL;
                                COMMENT ON COLUMN app_shops_datas.dat_01hq0004_products.price_dot_arr IS 'ข้อมูลราคาขายสินค้าแต่ละ dot เก็บเป็น Array';
