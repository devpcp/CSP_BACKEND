...
const modelShopLegacySalesOut = require("../../models/ShopLegacySalesOut/ShopLegacySalesOut");
const migrateCreateTableShopLegacySalesOut = async ({ transaction }) => {
    const transactionResults = await xSequelize.transaction(
        ...
        async (transaction) => {
            ...
            for (let index = 0; index < shop_code_ids.length; index++) {
                ...
                const instanceShopPurchaseOrderDoc = modelShopLegacySalesOut(element_shop_code_id);
...
migrateCreateTableShopLegacySalesOut({ transaction: null });

module.exports = migrateCreateTableShopLegacySalesOut;