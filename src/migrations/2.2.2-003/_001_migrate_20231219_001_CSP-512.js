    ...
    const transactionResults = await db.transaction(
        ...
        async (transaction) => {
            for (let indexShop = 0; indexShop < shop_code_ids.length; indexShop++) {
                ...
                await ShopCustomerDebtDebitNoteDoc(table_name).sync({
                    force: true,
                    transaction: transaction
                });

                await ShopCustomerDebtDebitNoteList(table_name).sync({
                    force: true,
                    transaction: transaction
                });

                await ShopCustomerDebtCreditNoteDoc(table_name).sync({
                    force: true,
                    transaction: transaction
                });

                await ShopCustomerDebtCreditNoteList(table_name).sync({
                    force: true,
                    transaction: transaction
                });
            }
