...
const migrate_ProductPurchaseUnitTypes_HaveRunNumber = async () => {
    const transactionResults = await xSequelize.transaction(
        ...
        async (transaction) => {
            await xSequelize.query(
                `ALTER TABLE master_lookup.mas_product_purchase_unit_types ADD run_no integer;`,
                {
                    type: QueryTypes.RAW,
            ...
            await xSequelize.query(
                `COMMENT ON COLUMN master_lookup.mas_product_purchase_unit_types.run_no IS 'เลขที่ run เอกสาร';`,
                {
                    type: QueryTypes.RAW,
                    transaction: transaction
                }
            ...
            await xSequelize.query(
                `ALTER TABLE master_lookup.mas_product_purchase_unit_types ADD internal_code_id varchar NULL;`,
                {
                    type: QueryTypes.RAW,
                    transaction: transaction
                }
            ...
            await xSequelize.query(
                `COMMENT ON COLUMN master_lookup.mas_product_purchase_unit_types.code_id IS 'รหัสควบคุมประเภทยานพาหนะ (ภายใน)';`,
                {
                    type: QueryTypes.RAW,
                    transaction: transaction
                }
            ...
            await xSequelize.query(
                `
                UPDATE master_lookup.mas_product_purchase_unit_types AS A
                SET internal_code_id = B.code_id
                FROM master_lookup.mas_product_purchase_unit_types AS B
                WHERE A.id = B.id;
                `,
                {
                    type: QueryTypes.RAW,
            ...
            await xSequelize.query(
                ...
                DECLARE USERDefined_DN_DEFAULT VARCHAR := '${config.config_run_number_master_product_purchase_unit_types_prefix}';
                ...
                BEGIN    
                    ...
                    FOR FFF IN
                        SELECT extract(year FROM A.created_date at time zone USERDefined_Timezone)::VARCHAR(4) AS LAPPOINT_YEAR
                            FROM master_lookup.mas_product_purchase_unit_types AS A
                    ...
                    LOOP
                        ...
                        FOR III IN
                            SELECT B.id, B.created_date FROM master_lookup.mas_product_purchase_unit_types AS B
                        ...
                        LOOP
                            ...
                            UPDATE master_lookup.mas_product_purchase_unit_types AS AX
            ...
            await xSequelize.query(
                `ALTER TABLE master_lookup.mas_product_purchase_unit_types ALTER COLUMN code_id SET NOT NULL;`,
                {
                    type: QueryTypes.RAW,
                    transaction: transaction
                }
            ...
            await xSequelize.query(
                `ALTER TABLE master_lookup.mas_product_purchase_unit_types ALTER COLUMN run_no SET NOT NULL;`,
                {
                    type: QueryTypes.RAW,
                    transaction: transaction
                }
...
module.exports = migrate_ProductPurchaseUnitTypes_HaveRunNumber;
