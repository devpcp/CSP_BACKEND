...
/**
 * @typedef {{doc_type_code?: 'INI'|'SO'|'WYZAuto';}} IFieldDocTypeCode - An additional field to explain what doc type is.
 */

/**
 *
 * @param {string} shop_code - table_name
 * @param {Object?} options - Filter options
 * @param {string?} options.filter_by_productId - Option: Filter by ProductId
 * @param {string?} options.filter_by_warehouseId - Option: Filter By WarehouseId
 * @param {string?} options.filter_by_warehouseItemId - Option: Filter by WarehouseItemId
 * @param {string?} options.filter_by_DOT - Option: Filter by DOT
 * @param {string?} options.filter_by_purchaseUnitId - Option: Filter by PurchaseUnitId
 * @returns {Promise<Array<import('../types/type.Handler.ShopReports.InventoryMovements').IInventoryMovement & IFieldDocTypeCode>>}
 */
const utilGetInventoryMovementsFromLogs = async (shop_code, options) => {
    const functionName = utilGetInventoryMovementsFromLogs.name;

    if (!_.isString(shop_code) || shop_code.length <= 0) {
        throw Error(`${functionName}: shop_code require String`);
    }
    else {
    ...
    /**
     * Option: Filter by ProductId
     * @type {string}
     */
    const filter_by_productId = _.get(options, 'filter_by_productId', null);
    if (_.isString(filter_by_productId)) {
        if (!isUUID(filter_by_productId)) {
            throw Error(`${functionName}: options.filter_by_productId require Empty or String UUID`);
        }
    }
    /**
     * Option: Filter By WarehouseId
     * @type {string}
     */
    const filter_by_warehouseId = _.get(options, 'filter_by_warehouseId', null);
    if (_.isString(filter_by_warehouseId)) {
        if (!isUUID(filter_by_warehouseId)) {
            throw Error(`${functionName}: options.filter_by_warehouseId require Empty or String UUID`);
        }
    }
    /**
     * Option: Filter by WarehouseItemId
     * @type {string}
     */
    const filter_by_warehouseItemId = _.get(options, 'filter_by_warehouseItemId', null);
    if (_.isString(filter_by_warehouseItemId)) {
        if (filter_by_warehouseItemId.length <= 0) {
            throw Error(`${functionName}: options.filter_by_warehouseId require Empty or String length more than 0`);
        }
    }
    /**
     * Option: Filter by DOT
     * @type {string}
     */
    const filter_by_DOT = _.get(options, 'filter_by_DOT', null);
    if (_.isString(filter_by_warehouseItemId)) {
        if (filter_by_warehouseItemId.length !== 4) {
            throw Error(`${functionName}: options.filter_by_warehouseId require Empty or String length equal 4`);
        }
    }
    /**
     * Option: Filter by PurchaseUnitId
     * @type {string}
     */
    const filter_by_purchaseUnitId = _.get(options, 'filter_by_purchaseUnitId', null);
    if (_.isString(filter_by_warehouseId)) {
        if (!isUUID(filter_by_warehouseId)) {
            throw Error(`${functionName}: options.filter_by_purchaseUnitId require Empty or String UUID`);
        }
    }

    const ModelProductMovementLog_INI = sequelize.define('ProductMovementLog', {
        "id": {
            field: '_0',
            type: DataTypes.UUID,
            primaryKey: true,
    ...
    const ModelProductMovementLog_SO = sequelize.define('ProductMovementLog', {
        "id": {
            field: '_0',
            type: DataTypes.UUID,
            primaryKey: true,
    ...
    const ModelProductMovementLog_WYZAuto = sequelize.define('ProductMovementLog', {
        "id": {
            field: '_0',
            type: DataTypes.UUID,
            primaryKey: true,
            ...
            ...(await Promise.all([
                sequelize.query(
                    `
                        ...
                        FROM app_shops_datas.dat_${shop_code}_inventory_management_logs AS logs
                            CROSS JOIN LATERAL json_array_elements(logs.warehouse_detail) AS stock_warehouse
                            JOIN app_shops_datas.dat_${shop_code}_inventory_transaction_doc AS shop_transaction_doc ON logs.doc_inventory_id = shop_transaction_doc.id
                            JOIN master_lookup.mas_document_types AS mas_document_types ON mas_document_types.id = shop_transaction_doc.doc_type_id
                            JOIN app_shops_datas.dat_${shop_code}_business_partners AS shop_business_partner ON shop_transaction_doc.bus_partner_id = shop_business_partner.id
                            JOIN app_shops_datas.dat_${shop_code}_products AS shop_product ON logs.product_id = shop_product.id
                ...
                sequelize.query(
                    `
                        ...
                        FROM app_shops_datas.dat_${shop_code}_sales_order_plan_logs AS logs
                            CROSS JOIN LATERAL json_array_elements(logs.warehouse_detail->'shelf') AS stock_warehouse
                            JOIN app_shops_datas.dat_${shop_code}_sales_transaction_doc AS shop_sales_transaction_doc ON logs.doc_sale_id = shop_sales_transaction_doc.id
                            ...
                            LEFT JOIN app_shops_datas.dat_${shop_code}_personal_customers AS shop_personal_customer ON shop_sales_transaction_doc.per_customer_id = shop_personal_customer.id
                            LEFT JOIN app_shops_datas.dat_${shop_code}_business_customers AS shop_business_customer ON shop_sales_transaction_doc.bus_customer_id = shop_business_customer.id
                            JOIN app_shops_datas.dat_${shop_code}_products AS shop_product ON logs.product_id = shop_product.id
                            JOIN app_datas.dat_products AS hq_product ON shop_product.product_id = hq_product.id
                        WHERE logs.status = 1
                            ${!isUUID(filter_by_productId) ? '' : `AND logs.product_id = '${filter_by_productId}'`}
                    ...
                    `
                        WITH 
                            ...
                            SELECT
                                   ...
                                   'WYZAuto' AS "_19"
                            FROM CTE_3 AS AA
                            JOIN app_shops_datas.dat_${shop_code}_products AS shop_product ON AA.product_id = shop_product.id
                            JOIN app_datas.dat_products AS hq_product ON shop_product.product_id = hq_product.id
                            WHERE AA.status = 1
...
module.exports = utilGetInventoryMovementsFromLogs;