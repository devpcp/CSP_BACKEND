 ...
 * }}
 */
const utilProportionDiscountCalculator = (billAndLists, options = {}) => {
    ...
    if (Number(totalInlineProportionDiscountRatio) !== 1) {
        ...
        if (Number(totalInlineProportionDiscountRatio__Recheck) !== 1) {
            throw new Error(`ค่า Proportion discount ratio ต้องมากกว่าหรือเท่ากับ 0 และไม่เกิน 1 จากการเติมความถี่ส่วนลดตามสัดส่วน ที่ขาดหายไป: รายการที่ (${billAndLists_data.lists[findIndexOfMaxProppotion]?.seq_number || findIndexOfMaxProppotion}), ความถี่ส่วนลดตามสัดส่วนที่เติมไปแล้ว (${billAndLists_data.lists[findIndexOfMaxProppotion].proportion_discount_ratio})`);
        }
    }
    ...
    if (Number(totalInlineProportionDiscountPrice) !== Number(billAndLists_data.price_bill_discount)) {
        ...
        if (Number(totalInlineProportionDiscountPrice__ReCheck) !== Number(billAndLists_data.price_bill_discount)) {
            throw Error(`ค่า Proportion discount price ไม่สอดคล้องกันกับส่วนลดท้ายบิล จากการเติมราคาส่วนลดตามสัดส่วน ที่ขาดหายไป: รายการที่ (${billAndLists_data.lists[findIndexOfMaxProppotion]?.seq_number || findIndexOfMaxProppotion}), ราคาส่วนลดตามสัดส่วนที่เติมไปแล้ว (${billAndLists_data.lists[findIndexOfMaxProppotion].proportion_discount_price})`);
        }
    }
...
module.exports = utilProportionDiscountCalculator;